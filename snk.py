...
def point_inside(point, min_point, max_point):
	return (
		(point.x >= min_point.x) and (point.y >= min_point.y)
	) and ((point.x <= max_point.x) and (point.y <= max_point.y))
...
class Snake:
	...
	def set_barrier(
		...
		snake = self.body + make_line(
			self.direction,
			next_point(self.direction, self.get_head()),
			len(self.body) * 2,
		)
		area_min = Point(
			self.arena_min.x + barrier_margin,
			self.arena_min.y + barrier_margin,
		)
		area_max = Point(
			self.arena_max.x - barrier_margin,
		...
		while i != barrier_num:
			...
			if (
				inside
				and not list_match(snake, rand_line)
				and not list_match(self.barrier, rand_line)
			):
	...
	def crash(self):
		head = self.get_head()
		if (
			not point_inside(head, self.arena_min, self.arena_max)
			or head in self.barrier
...
def main(stdscr):
	...
	while True:
		...
		time.sleep(timing)
	while 1:
		go_msg = "".join((" Game over! Score:", str(score), " "))
		try:
			stdscr.addstr(
				(curses.LINES // 2) + 1,
				(curses.COLS // 2) - (len(go_msg) // 2),
				go_msg,
				curses.A_REVERSE,
			)
		except curses.error:
